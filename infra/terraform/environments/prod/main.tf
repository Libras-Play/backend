...
terraform {
  backend "s3" {
    bucket         = "aplicacion-senas-terraform-state"
    key            = "prod/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    dynamodb_table = "aplicacion-senas-terraform-locks"
  }
}
...
module "sns" {
  ...
  email_subscriptions = {
    "system-alerts" = "ops-team@example.com"
  }
  tags = local.common_tags
}
...
module "ecs" {
  ...
  services = {
    content-service = {
      ...
      autoscaling_max      = 4
      environment = {
        DATABASE_URL = "postgresql://postgres@${module.rds.cluster_endpoint}:5432/content_db"
